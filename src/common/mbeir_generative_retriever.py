...
def generate_codes_for_dataset(
    ...
    for batch in data_loader:
        ...
        # Candidate encoding and query generation have different valid shapes:
        #   quantizer/candidate path: [B, L]
        #   generator/query path:     [B, num_beams, L]
        # Preserve the leading batch dimension for singleton batches without
        # collapsing the query beam dimension.
        batch_ids = ids_list_batched.view(-1, 1).to(device)

        if is_quantizer:
            if codes_batched.ndim == 1:
                codes_batched = codes_batched.unsqueeze(0)
            if codes_batched.ndim != 2:
                raise RuntimeError(
                    "Expected candidate code tensor [B, L], "
                    f"got shape={tuple(codes_batched.shape)}"
                )
        else:
            # Defensive compatibility for a generator that may squeeze B=1
            # and return [num_beams, L]. The current GENIUS path normally
            # already returns [1, num_beams, L].
            if (
                codes_batched.ndim == 2
                and batch_ids.size(0) == 1
                and codes_batched.size(0) == num_beams
            ):
                codes_batched = codes_batched.unsqueeze(0)
            if codes_batched.ndim != 3:
                raise RuntimeError(
                    "Expected query beam tensor [B, num_beams, L], "
                    f"got shape={tuple(codes_batched.shape)}"
                )
            if codes_batched.size(1) != num_beams:
                raise RuntimeError(
                    "Unexpected query beam count: "
                    f"expected={num_beams} got={codes_batched.size(1)} "
                    f"shape={tuple(codes_batched.shape)}"
                )

        if codes_batched.size(0) != batch_ids.size(0):
            raise RuntimeError(
                "Code/ID batch-size mismatch: "
                f"codes={tuple(codes_batched.shape)} ids={tuple(batch_ids.shape)}"
            )

        codes_tensor.append(codes_batched)
        id_tensor.append(batch_ids)
        if use_embedding:
            if encode_batched.ndim == 1:
                encode_batched = encode_batched.unsqueeze(0)
            if encode_batched.size(0) != batch_ids.size(0):
                raise RuntimeError(
                    "Embedding/ID batch-size mismatch: "
                    f"embeddings={tuple(encode_batched.shape)} ids={tuple(batch_ids.shape)}"
                )
    ...
    for split_name, split_dir, dataset_name_list, cand_pool_name_list in splits:
        for dataset_name, cand_pool_name in zip(dataset_name_list, cand_pool_name_list):
            ...
            if split_name == "cand_pool" and is_extracted:
                ...
                query_generation_start = time.perf_counter()
                ...
                if torch.cuda.is_available():
                    torch.cuda.synchronize()
                query_generation_seconds = time.perf_counter() - query_generation_start
                if dist_utils.is_main_process():
                    print(
                        "Generation Metrics: "
                        f"seconds={query_generation_seconds:.6f} "
                        f"examples={len(dataset)} "
                        f"examples_per_second={len(dataset) / max(query_generation_seconds, 1e-12):.6f}"
                    )
