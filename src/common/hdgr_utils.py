...
def build_model_from_config(config):
    ...
    if model_name == "CLIPScoreFusion":
        ...
        model.load_state_dict(torch.load(checkpoint_path, map_location=torch.device('cpu'), weights_only=False)["model"])
        ...
        model.load_state_dict(gen_ckpt["model"], strict=False)
        
    elif model_name in ("T5GenerativeRetriever", "HDGRBlockDenoisingRetriever", "HDGR_block_denoising", "GPTDiffusionRetriever", "GPT_diffusion", "GPTHDGRBackboneRetriever", "GPT_HDGR", "GPTRetrievalGoodRetriever", "GPT_RetrievalGood", "GPTRetrievalGood", "GENIUSOfficialAR", "GENIUS_AR", "HDGR_BD3Faithful", "HDGR_BD3Light", "HDGRBD3Faithful", "HDGR_BD3_Faithful", "HDGR_BD3Faithful_FullID", "HDGR_BD3Faithful_2Stage") or (("BD3Faithful" in str(model_name)) or ("BD3Light" in str(model_name))) or ("BD3_Faithful" in str(model_name)):
        from models.uniir_clip.clip_nofusion.clip_nf import CLIPNoFusion
        ...
        if not uses_t5_tokenizer(config):
            ...
            if is_gpt_hdgr_config(config):
                print("[Tokenizer] GPT-HDGR backbone eval: using internal code tokenizer; skipping T5 tokenizer.")
            elif is_genius_ar_config(config):
                print("[Tokenizer] GENIUS_AR official T5-style eval: using internal code tokenizer; skipping external T5 tokenizer.")
            else:
                raise ValueError("Only GENIUS AR and Code-Tied HDGR are retained.")
        else:
        ...
        model = RetrieverClass(config=config, tokenizer=seq2seq_tokenizer, clip_model=clip_model)

        ckpt_config = model_config.ckpt_config
        checkpoint_path = os.path.join(config.genir_dir, ckpt_config.ckpt_dir, ckpt_config.ckpt_name)
        assert os.path.exists(checkpoint_path), f"Checkpoint file {checkpoint_path} does not exist."
        print(f"loading generative retriever checkpoint from {checkpoint_path}")
        gen_ckpt = torch.load(
            checkpoint_path,
            map_location=torch.device('cpu'),
            weights_only=False
