...
class T5ForGenerativeRetrieval(nn.Module):
    ...
    def generative_index(self, cand_codes):
        ...
        trie_type = str(self.trie_type).replace('_', '').lower()
        if trie_type == 'marisa':
            trie = MarisaTrie(cand_codes_ids)
        elif trie_type == 'triecpp' and Trie_Cpp is not None:
            trie = Trie_Cpp.Trie(cand_codes_ids)
        else:
            if trie_type == 'triecpp' and Trie_Cpp is None and utils.is_main_process():
                print("WARNING: trie_cpp extension not found; using pure Python Trie.")
    ...
    def distribute_trie(self, cand_codes, trie_save_path):
        ...
        trie_type = str(self.trie_type).replace('_', '').lower()
        use_trie_cpp = trie_type == 'triecpp' and Trie_Cpp is not None
        if trie_type == 'triecpp' and Trie_Cpp is None and utils.is_main_process():
            print("WARNING: trie_cpp extension not found; using pure Python Trie for distribute_trie.")
