...
def save_checkpoint(model, optimizer, scheduler, epoch, scaler, config, latest_overwrite=True):
    ckpt_config = config.model.ckpt_config
    model_name = config.model.short_name.lower()
    if latest_overwrite:
        checkpoint_name = f"{model_name}_latest.pth"
    else:
    ...
    if latest_overwrite:
        print(f"Saved latest checkpoint to {checkpoint_path} (epoch={epoch}, overwrote previous latest)")
    else:
...
def train(
    ...
    for epoch in range(epoch, config.trainer_config.num_train_epochs):
        ...
        # Save latest checkpoint after every epoch, overwriting the previous one.
        if utils.is_main_process():
            save_checkpoint(model_without_ddp, optimizer, scheduler, epoch, scaler, config, latest_overwrite=True)
...
def main(config):
    ...
    if enable_eval:
        ...
        # Stage-2 train uses pre-extracted query/pool features.
        # CLIP is unused when validation is disabled.
        del clip_model
        clip_model = None
        gc.collect()
        print("[GENIUS low-RAM] Released unused CLIP model.")
