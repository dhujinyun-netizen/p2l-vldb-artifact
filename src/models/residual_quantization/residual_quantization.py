...
class RQ(nn.Module):
    ...
    def encode_extracted_batch(self, batch: Tuple, code_output: bool = False, 
        ...
        # Prepare masks and embeddings.
        #
        # Extracted CLIP-SF features may be stored as FP16, while the RQ
        # encoder is normally kept in FP32 during SoundStorm evaluation.
        # Match both device and dtype to the quantizer encoder parameters.
        ref_param = next(self.encoder.parameters())
    ...
    def inference(self, img_emb: torch.Tensor, txt_emb: torch.Tensor, 
        ...
        # Always preserve the leading batch dimension, including B=1.
        # Candidate pools whose size is not divisible by the evaluation batch
        # size produce a singleton final batch. Squeezing it changes `Is` from
        # [1, code_length] to [code_length] and breaks concatenation with prior
        # batches in the generative-retriever evaluation pipeline.
