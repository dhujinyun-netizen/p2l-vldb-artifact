...
def main(config):
    ...
    for i, batch in enumerate(tqdm(query_loader)):
        ...
        # In single-GPU mode the original code skipped the distributed gather block,
        # leaving q_text_mask/q_image_mask undefined. Keep local tensors as defaults
        # and overwrite them only when multi-process gathering is actually needed.
    ...
    if utils.is_main_process():
        ...
        # Free cached CUDA blocks before candidate-pool extraction.
        gc.collect()
    ...
    for i, batch in enumerate(tqdm(cand_loader)):
        ...
        # Same single-GPU fallback for candidate masks.
