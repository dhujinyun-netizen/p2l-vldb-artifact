...
def generate_embeds_for_config(model, img_preprocess_fn, tokenizer, config):
    ...
    # Use the configured candidate pools when constructing the union embedding.
    # The original script hard-coded the full M-BEIR pool list, which breaks
    # Flickr30K-only local reproduction.
    all_cand_pool_name_list = list(getattr(embed_config.cand_pools_config, "cand_pools_name_to_embed", []))
